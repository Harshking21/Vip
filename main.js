...
const token = "7273976322:AAGkGzq87YHP2GgqX-ZDhjvll9H9RSqX5Xs";
admin_account = [6436260909];
USER_FILE = "users.txt";
LOG_FILE = "log.txt";
// Create a new bot instance
const bot = new TelegramBot(token, { polling: true });
let message;
publicMembers = [1920313123];
premiumMembers = [];
  ...
  try {
    if (!premiumMembers.includes(chatId) && !admin_account.includes(chatId)) {
      const message = await bot.sendMessage(
        chatId,
        `Dear ${msg.chat.first_name},\nYou're Not a Premium Member,\nBuy Premium from: @KONG_MODS_OWNER`
      );
